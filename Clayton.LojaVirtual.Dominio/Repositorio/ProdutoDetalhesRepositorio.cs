...
{
    public class ProdutoDetalhesRepositorio
    {
        ...
        public DetalhesProdutoDto ObterProdutoModelo(string codigo, string corCodigo)
        {
            ...
            var query = from p in _context.ProdutosDetalhes
            ...
            var produto = (from q in query where q.CorCodigo == corCodigo select q).FirstOrDefault();



            DetalhesProdutoDto dto = new DetalhesProdutoDto
            {
                Produto = Mapper.DynamicMap<ProdutoDetalhes>(produto),
